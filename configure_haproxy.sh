...
for i in `seq 1 $((NUMBER_OF_NODES))`; do
	...
	docker exec node$i /bin/bash -c "echo 'mysqlchk	9200/tcp	# mysqlchk' >> /etc/services "
	echo "	- configuring Cluster Check account!"
	docker exec node$i /bin/bash -c "mysql -u root -proot -e \"grant process on *.* to 'clustercheckuser'@'%' identified by 'clustercheckpassword!';flush privileges;\" "
	echo "	- configuring MySQL Check script!"
